...
export default function EditContentPage() {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            try {
                const response = await contentsApi.getAll({ limit: 100 });
                const contents = response.data || response || [];
                const content = contents.find((c: { id: string }) => c.id === contentId);
                
                if (content) {
                    const translations: Record<string, { title: string; body: string; excerpt: string }> = {
                        fr: { title: '', body: '', excerpt: '' },
                        en: { title: '', body: '', excerpt: '' },
                        ar: { title: '', body: '', excerpt: '' },
                    };

                    content.translations?.forEach((t: ContentTranslation) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error('Content update error:', axiosErr.response?.data);
        ...
        <div>
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <h1 className="text-2xl font-semibold text-neutral-900">Modifier le contenu</h1>
                </div>
            </div>
